...
def process_file(input_path, output_path):
    try:
        ...
        # Verifica que el número de columnas coincida con el número de nombres que se van a asignar
        expected_columns = ['Dia', 'Fecha', 'Nombre', 'Horario', 'Marca1','Marca2','Marca3','Marca4','Marca5','Marca6','DNI','Marca7','FISCALIZADO']
        
        if len(df.columns) == len(expected_columns):
            df.columns = expected_columns
        else:
            raise ValueError(f"Expected {len(expected_columns)} columns, but got {len(df.columns)}")
        ...
        #df = df.dropna(subset=['Marca1', 'Marca2', 'Marca3', 'Marca4','Marca5','Marca6','Marca7'], how='all')
